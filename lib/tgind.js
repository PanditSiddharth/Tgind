...
class Tgind extends events_1.EventEmitter {
    /**
     *
     * @param {string} TOKEN
            ...
            while (this.run) {
                ...
                if (response.ok && response.result.length > 0) {
                    ...
                    response.result.forEach((update) => {
                        ...
                        updt.update_id = update.update_id;
                        let short;
                        short = new short_1.TgindShort(this.TOKEN, updt);
                        setTimeout(() => {
                            short = null;
                        }, this.options.timeout);
                        const methodNames = Object.keys(short);
                        methodNames.forEach((methodName) => {
                            if (methodName !== 'constructor' && typeof short[methodName] === 'function') {
                                Object.defineProperty(updt, methodName, {
                                    value: short[methodName],
                                    enumerable: false
                                });
                            }
                        });
                        let util = new util_1.Util(updt);
                        this.emit(evnt, updt, util);
